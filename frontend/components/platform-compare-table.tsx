...
const PLATFORM_ORDER = ['instagram_reels', 'youtube_shorts', 'linkedin']

const SECTION_LABELS = [
  { key: 'hook',              label: 'Hook',        sublabel: '0-3s' },
  { key: 'angle',             label: 'Angle',       sublabel: 'framing' },
  { key: 'script.opening',    label: 'Opening',     sublabel: '0-3s' },
  { key: 'script.body',       label: 'Body',        sublabel: '3-50s' },
  { key: 'script.closer',     label: 'Closer',      sublabel: 'landing' },
  { key: 'cta',               label: 'CTA',         sublabel: 'close' },
  { key: 'higgsfield_prompt', label: 'Higgsfield',  sublabel: 'visual AI' },
  { key: 'editing_notes',     label: 'Edit Notes',  sublabel: 'editor' },
] as const
...
export function PlatformCompareTable({ derivatives, onUpdate }: Props) {
  ...
  async function handleRegen(d: Derivative) {
    setLoadingId(d.id)
    try {
      onUpdate(await api.derivatives.regenerate(d.id, guidance || undefined))
      setRegenId(null)
      setGuidance('')
    } finally { setLoadingId(null) }
  }
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-zinc-200">
      {/* Header */}
      <div className={`grid ${colClass} border-b border-zinc-100 bg-zinc-50/80`}>
        <div className="p-3" />
        {briefs.map(d => {
          ...
          return (
            <div key={d.id} className="border-l border-zinc-100 p-3 text-center">
              ...
              <div className="mt-1.5">
                {d.status === 'approved' && <span className="text-[10px] font-medium text-emerald-600">✓ Approved</span>}
                {d.status === 'rejected' && <span className="text-[10px] text-zinc-400">✗ Rejected</span>}
                {d.status === 'draft'    && <span className="text-[10px] text-zinc-400">Draft</span>}
              </div>
            </div>
          )
        })}
      ...
      {SECTION_LABELS.map(({ key, label, sublabel }) => (
        <div key={key} className={`grid ${colClass} border-b border-zinc-50 last:border-b-0`}>
          ...
          {briefs.map(d => {
            ...
            const isHook = key === 'hook'
            const isEditing = key === 'editing_notes'
            return (
              <div
                key={`${d.id}-${key}`}
                className={`border-l border-zinc-50 p-3 ${isVisual ? 'bg-violet-50/30' : ''} ${isEditing ? 'bg-amber-50/20' : ''}`}
              >
                <p className={`text-xs leading-relaxed ${
                  isVisual  ? 'font-mono text-violet-700' :
                  isEditing ? 'text-amber-800' :
                  isHook    ? 'font-semibold text-zinc-900' :
                  'text-zinc-700'
                }`}>
      ...
      {/* Actions */}
      <div className={`grid ${colClass} border-t border-zinc-200 bg-zinc-50/40`}>
        <div className="flex items-center border-r border-zinc-100 p-3">
          <span className="text-[10px] font-medium uppercase tracking-wide text-zinc-400">Actions</span>
        </div>
        {briefs.map(d => (
          <div key={`actions-${d.id}`} className="flex flex-wrap items-center gap-1.5 border-l border-zinc-100 p-3">
            {d.status !== 'rejected' && (
              <>
                {d.status !== 'approved' && (
                  <button onClick={() => handleApprove(d)} disabled={loadingId === d.id} className="btn-xs btn-primary">
                    Approve
                  </button>
                )}
                <button onClick={() => setRegenId(regenId === d.id ? null : d.id)} disabled={loadingId === d.id} className="btn-xs btn-ghost">
                  Regen
                </button>
                <button onClick={() => handleReject(d)} disabled={loadingId === d.id} className="btn-xs btn-ghost text-zinc-400">
                  Reject
                </button>
              </>
            )}
      ...
      {regenId && (
        <div className="border-t border-zinc-200 bg-white p-4">
          <div className="flex gap-2">
            ...
            />
            <button
              onClick={() => { const d = briefs.find(b => b.id === regenId); if (d) handleRegen(d) }}
              disabled={!!loadingId}
              className="btn-xs btn-primary"
            >
              {loadingId ? '...' : 'Regenerate'}
            </button>
            <button onClick={() => { setRegenId(null); setGuidance('') }} className="btn-xs btn-ghost">
              Cancel
            </button>
          </div>
        </div>
